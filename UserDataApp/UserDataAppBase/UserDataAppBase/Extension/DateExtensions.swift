...
public extension Date {
	...
	///
	/// - Parameter style: DateFormfunc atter style (default is .medium).
    ...
    func timeString(formatter dateFormatter: DateFormatter) -> String {
        return dateFormatter.string(from: self)
    }
    static func secondToTime(sec: Int) -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "hh aa"
        dateFormatter.locale = Locale(identifier: "en_US_POSIX")
        dateFormatter.timeZone = TimeZone(abbreviation: "GMT")
        return Date(timeIntervalSince1970: TimeInterval(sec)).timeString(formatter: dateFormatter)
    }
    static func secondToVideoTime(sec: Int) -> String {
        let dateFormatter = DateFormatter()
...
extension Date {
    ...
    func strPastMessageDate() -> String {
        if self.isInToday {
    ...
    func strPastTodayFutureDate() -> String {
        if self.isInToday {
            return "Today"
        } else if self.isInYesterday {
            return "Yesterday"
    ...
    func strPastTodayFutureTime() -> String {
        if self.isInToday {
            return "Today"
        } else if self.isInYesterday {
            return "Yesterday"
    ...
    func strPastTodayFutureDateAndTime() -> String {
        if self.isInToday {
            return "Today\(self.string(withFormat: "h:mm a"))"
        } else if self.isInYesterday {
            return "Yesterday\(self.string(withFormat: "h:mm a"))"
        } else if self.isInTomorrow {
            return "Tomorrow \(self.string(withFormat: "h:mm a"))"
        }else {
            return self.string(withFormat: "dd MMM,EEEE,h:mm a")
        }
    }

    func timeToGoDisplay() -> String {
        ...
        //            return "\(secondsAgo) seconds to go"
        //        } else
        if secondsAgo < 0{
            return "On Going"
        }else if secondsAgo < hour {
            if (secondsAgo / minute) == 0
            {
                return "On Going"
            }
            else
            {
                return "\(secondsAgo / minute) minutes to go"
            }
        } else if secondsAgo < day {
            return "\(secondsAgo / hour) hours to go"
        } else if secondsAgo < week {
            return "\(secondsAgo / day) days to go"
        }
        return "\(secondsAgo / week) weeks to go"
    }
